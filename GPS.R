...
GPS<-function(outfile_name='GPS_results.txt',
	          N_best=1, fname="data.csv",
	          directory_name="C://GPS//"){
  ...
  write("Population\tSample_no\tSample_id\tPrediction\tLat\tLon", 
  	    outfile_name, append=FALSE)
  ...
  for(GROUP in GROUPS){
    ...
    for(a in 1:K){
      ...
      write(paste(GROUP, a, row.names(Y[a,]), best_ethnic[1], 
      	          GEO[minG[1],1]+la1, GEO[minG[1],2]+lo1, sep="\t"), 
                  outfile_name, append=TRUE)
    }
  }
  return ("This GPS thing is done!")
}